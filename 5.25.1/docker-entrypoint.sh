...
if [ ! -z "$MONIT_VOLUME_EXCLUDED" ]; then
    GREP="| grep -v -E '($MONIT_VOLUME_EXCLUDED)'"
else
    GREP=""
fi

MOUNTS=$(findmnt -n -lo target -t btrfs,ext4,nfs,xfs | egrep '^/host' | egrep -v '^/host/var/lib/docker/devicemapper' $GREP || true)
...
if [ ! -z "$MONIT_NETWORK_EXCLUDED" ]; then
    GREP="| grep -v -E '($MONIT_NETWORK_EXCLUDED)'"
else
    GREP=""
fi

INTERFACES=$(ifconfig | cut -c 1-8 | sort | uniq -u -E $GREP)
...
if [ ! -z "$MONIT_CONTAINER_EXCLUDED" ]; then
    GREP="| grep -v -E '($MONIT_CONTAINER_EXCLUDED)'"
else
    GREP=""
fi
...
CONTAINERS=$(docker ps --format "{{.Label \"$MONIT_CONTAINER_LABEL\"}}") $GREP
...
if [ ! -z "$MONIT_DB_DATA_PATH" -a -d "$MONIT_DB_DATA_PATH" ]; then
    ...
    if [ ! -z "$MONIT_DB_EXCLUDED_PATH" ]; then
        EXPLUDE_PATH="| grep -v -E '($MONIT_DB_EXCLUDED_PATH)'"
    else
        EXPLUDE_PATH=""
    fi
    ...
    DB_FILES=$(ls -aRm $MONIT_DB_PATH.{$INCLUDE_EXT} $EXPLUDE_PATH | tr "," " ")
